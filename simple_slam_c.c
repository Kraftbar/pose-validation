...
static uint32_t fast_rand_u32(void) {
...
static int map_point_is_active(const Map *map, int idx) {
    return idx >= 0 && idx < map->size && map->data[idx].active;
}

static int project_map_point(const Pose *pose, const MapPoint *p, double fx, double fy, double cx,
                             double cy, double *u, double *v, double *z) {
    double R[9], t[3], cp[3];
    pose_get_rotation(pose, R);
    pose_get_translation(pose, t);
    ...
    if (!isfinite(cp[0]) || !isfinite(cp[1]) || !isfinite(cp[2]) || cp[2] <= 1e-6)
        return 0;
    *z = cp[2];
    *u = fx * cp[0] / cp[2] + cx;
    *v = fy * cp[1] / cp[2] + cy;
...
static double reproj_error_sq(const Pose *pose, const MapPoint *p, Corner obs, double fx, double fy,
                              double cx, double cy) {
    double u, v, z, dx, dy;
    if (!project_map_point(pose, p, fx, fy, cx, cy, &u, &v, &z))
        return DBL_MAX;
    dx = u - obs.x;
    dy = v - obs.y;
    return dx * dx + dy * dy;
...
static int triangulate_map_point_checked(const Pose *p1, const Pose *p2, Corner pt1, Corner pt2,
                                         double fx, double fy, double cx, double cy, double X[3],
                                         double *avg_err) {
    if (!triangulate_point(p1, p2, pt1, pt2, fx, fy, cx, cy, X))
        return 0;
    if (avg_err)
        *avg_err = 0.0;
    return 1;
...
static void sanitize_corners_with_map(CornerVec *corners, const Map *map) {
    for (int i = 0; i < corners->size; i++)
        if (corners->data[i].pt_idx != -1 && !map_point_is_active(map, corners->data[i].pt_idx))
            corners->data[i].pt_idx = -1;
}

static void observe_tracked_map_points(Map *map, CornerVec *corners, const Pose *pose, double fx,
                                       double fy, double cx, double cy) {
    for (int i = 0; i < corners->size; i++) {
        int idx = corners->data[i].pt_idx;
        double err2;
        if (!map_point_is_active(map, idx)) {
            corners->data[i].pt_idx = -1;
            continue;
        }
        err2 = reproj_error_sq(pose, &map->data[idx], corners->data[i], fx, fy, cx, cy);
        if (isfinite(err2) && err2 < 25.0) {
            map->data[idx].obs++;
            map->data[idx].err_sum += sqrt(err2);
        }
    }
}

static double pose_reprojection_cost(const Map *map, const CornerVec *corners, const Pose *pose,
                                     double fx, double fy, double cx, double cy) {
    double cost = 0.0;
    int used = 0;
    for (int i = 0; i < corners->size; i++) {
        double err2;
        if (!map_point_is_active(map, corners->data[i].pt_idx))
            continue;
        err2 = reproj_error_sq(pose, &map->data[corners->data[i].pt_idx], corners->data[i], fx, fy,
                               cx, cy);
        if (!isfinite(err2) || err2 == DBL_MAX)
            continue;
...
static int estimate_pose_PnP(const Map *map, const CornerVec *corners, double fx, double fy,
    ...
    for (int i = 0; i < corners->size; i++)
        if (map_point_is_active(map, corners->data[i].pt_idx) &&
            map->data[corners->data[i].pt_idx].obs >= 1)
            n++;
    if (n < 12)
        return 0;
    ...
    for (int i = 0; i < corners->size; i++)
        if (map_point_is_active(map, corners->data[i].pt_idx) &&
            map->data[corners->data[i].pt_idx].obs >= 1)
            ids[k++] = i;
    double best_P[12], best_err = DBL_MAX;
    int best_inl = 0;
    for (int it = 0; it < 150; it++) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            do {
                dup = 0;
                idx = ids[fast_rand_u32() % (uint32_t)n];
                for (int j = 0; j < i; j++)
                    if (sample[j] == idx) {
                        dup = 1;
                        break;
        ...
        if (inl > best_inl || (inl == best_inl && inl > 0 && err_sum < best_err)) {
            best_inl = inl;
            best_err = err_sum;
    ...
    for (int iter = 0; iter < 10; iter++) {
            ...
            break;
        {
            ...
            double dr[9] = {1, -dx[5], dx[4],
                            dx[5], 1, -dx[3],
                            -dx[4], dx[3], 1};
            double Rn[9], cand_R[9];
            Pose cand_pose;
            mat3_mul(dr, R, Rn);
            project_to_SO3(Rn, cand_R);
            pose_from_rt(cand_R, cand_t, &cand_pose);
            {
                double cand_cost = pose_reprojection_cost(map, corners, &cand_pose, fx, fy, cx, cy);
                if (cand_cost < best_cost) {
                    memcpy(R, cand_R, sizeof(cand_R));
                    memcpy(t, cand_t, sizeof(cand_t));
                    best_cost = cand_cost;
                    lambda = fmax(1e-5, lambda * 0.5);
...
static void local_ba(KFDB *db, Map *map, double fx, double fy, double cx, double cy) {
    ...
    for (int iter = 0; iter < 5; iter++) {
        ...
        for (int i = 0; i < map->size; i++) {
            if (!map->data[i].active || map->data[i].obs < 2)
                continue;
            double H[9] = {0}, b[3] = {0};
            for (int k = db->size - 3; k < db->size; k++) {
                ...
                for (int j = 0; j < kf->corners.size; j++) {
                    if (kf->corners.data[j].pt_idx != i || !map->data[i].active)
                        continue;
...
int main(int argc, char **argv) {
    ...
    while (frame_id < (int)(25.0 * cfg.seconds) && ffmpeg_read(cap, raw)) {
        ...
        if (frame_id == 0) {
            extract_corners_pure(cblur, w, h, &curr.corners, target_corners);
            mkf = 1;
            pose_identity(&pose);
        } else {
            track_corners_pure_lk(pgray, cblur, w, h, &prev.corners, &tracked, &matches);
            sanitize_corners_with_map(&tracked, &map);
            ...
            if (mkf) {
                if (inl >= 8) {
                    for (int j = 0; j < matches.size; j++) {
                        double X[3], avg_err = 0;
                        if (!mask || !mask[j])
                            continue;
                        if (tracked.data[matches.data[j].train_idx].pt_idx == -1) {
                            if (triangulate_map_point_checked(
                                ...
                                map_push(&map, (MapPoint){X[0], X[1], X[2], avg_err * 2.0, 2, 1});
                                pts++;
                                added++;
                                tri++;
                            }
                        } else if (map_point_is_active(
                                       &map, tracked.data[matches.data[j].train_idx].pt_idx)) {
                            map.data[tracked.data[matches.data[j].train_idx].pt_idx].obs++;
                        }
                    }
                }
                if (tracked.size < 600)
                    extract_corners_pure(cblur, w, h, &tracked, target_corners);
                unsigned char thumb[256];
