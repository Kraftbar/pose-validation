...
static cv::Matx33d poseRotation(const cv::Matx44d &T) {
    return cv::Matx33d(T(0, 0), T(0, 1), T(0, 2), T(1, 0), T(1, 1), T(1, 2), T(2, 0), T(2, 1),
                       T(2, 2));
}
...
class SimpleSlamOpt {
        ...
        if (fid > 0) {
            ...
            if (config_.use_pnp && !prev.uv_to_point.empty()) {
                ...
                for (const auto &match : matches) {
                    ...
                    world_pts.emplace_back(static_cast<float>(mp.pt.x), static_cast<float>(mp.pt.y),
                                           static_cast<float>(mp.pt.z));
            ...
            if (!used_pnp) {
                ...
                if (estimatePoseE(prev.kps, kps, matches, K, rel, mask)) {
                    inliers = static_cast<int>(
                        std::count(mask.begin(), mask.end(), static_cast<uchar>(1)));
  ...
  private:
    ...
    void extractFeatures(const cv::Mat &image, std::vector<cv::Point2f> &kps_out,
                         cv::Mat &des_out) const {
        cv::Mat gray;
    ...
    bool estimatePoseE(const std::vector<cv::Point2f> &kps1, const std::vector<cv::Point2f> &kps2,
                       const std::vector<cv::DMatch> &matches, const cv::Matx33d &K,
                       cv::Matx44d &out_pose, std::vector<uchar> &out_mask) const {
        ...
        cv::Mat mask;
        const cv::Mat E =
            cv::findEssentialMat(pts1, pts2, cv::Mat(K), cv::RANSAC, 0.999, 1.5, mask);
        if (E.empty()) {
            return false;
        }
    ...
    bool solvePnp(const std::vector<cv::Point3f> &world_pts,
                  const std::vector<cv::Point2f> &image_pts, const cv::Matx33d &K,
                  cv::Matx44d &out_pose, int &out_inliers) const {
        out_pose = cv::Matx44d::eye();
        out_inliers = 0;
        ...
        cv::Mat tvec;
        cv::Mat inliers;
        const bool ok = cv::solvePnPRansac(world_pts, image_pts, cv::Mat(K), cv::noArray(), rvec,
                                           tvec, false, 100, 3.0, 0.99, inliers, cv::SOLVEPNP_AP3P);
    ...
    void triangulatePoints(const cv::Matx44d &pose1, const cv::Matx44d &pose2,
                           const std::vector<cv::Point2f> &pts1,
                           const std::vector<cv::Point2f> &pts2, const cv::Matx33d &K,
        ...
        const cv::Matx34d Rt1(pose1(0, 0), pose1(0, 1), pose1(0, 2), pose1(0, 3), pose1(1, 0),
                              pose1(1, 1), pose1(1, 2), pose1(1, 3), pose1(2, 0), pose1(2, 1),
                              pose1(2, 2), pose1(2, 3));
        const cv::Matx34d Rt2(pose2(0, 0), pose2(0, 1), pose2(0, 2), pose2(0, 3), pose2(1, 0),
                              pose2(1, 1), pose2(1, 2), pose2(1, 3), pose2(2, 0), pose2(2, 1),
                              pose2(2, 2), pose2(2, 3));
        ...
        for (int i = 0; i < pts4d.cols; ++i) {
            ...
            const cv::Vec3d X(pts4d.at<double>(0, i) / w, pts4d.at<double>(1, i) / w,
...
int main(int argc, char **argv) {
    try {
        ...
        const cv::Matx33d K(static_cast<double>(proc_w), 0.0, static_cast<double>(proc_w) / 2.0,
        ...
        const int max_frames =
            is_webcam ? std::numeric_limits<int>::max() : static_cast<int>(fps * cfg.seconds);
        ...
        std::cout << std::fixed << std::setprecision(3) << "Processed frames=" << slam.frameCount()
                  << ", points=" << slam.pointCount() << ", duration=" << duration << "s\n";
