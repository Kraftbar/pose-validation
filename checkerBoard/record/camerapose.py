    ...
    def track(self, loop=True, verbose=False, show_video=None):
        ...
        while not self._kill:
            ...
            if not ids is None and self.id_to_find in ids[0]:
                ...
                # NED 
                pos_camera[1], pos_camera[0] = pos_camera[0], --pos_camera[1]
                ...
                if show_video:
                    ...
                    #-- Obs bug her fordi den er mot roll er med klokken
                    pitch_camera,roll_camera, yaw_camera = self._rotationMatrixToEulerAngles(self._R_flip*R_tc)
                    ...
                    if key == ord(' '):
                        ...
                        save_snaps(self._cap,len(self.attPos_log),self.savefolder)
                        self.attPos_log.append([math.degrees(roll_camera),math.degrees(pitch_camera),
                                        math.degrees(yaw_camera),pos_camera[0], pos_camera[1], pos_camera[2]])
            ...
            if show_video:
                ...
                if key == ord('q'):
                    self._cap.release()
                    np.savetxt(self.savefolder+"/"+"attPos_log.csv",self.attPos_log, delimiter=",")
