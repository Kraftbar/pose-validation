...
for (int i = 0; i < imgList.size(); i++) {
    ...
    // image pros
        ...
        // ---- Testing -----
	        ...
	        cv::viz::WSphere           point3Dm2(testpoint2, 2, 30, cv::viz::Color::brown());

            //3
            cv::Point3d                testpoint3(X3(0),X3(1),0);
	        cv::viz::WSphere           point3Dm3(testpoint3 , 2, 30, cv::viz::Color::red());
...
//        cv::eigen2cv(P,P_cv);
      ...
      // ------------------- TESTING triang ------------------- 
       ...
       // ------------------- TESTING ------------------- 
    	...
    	auto testPointt = triangulateOrb(g_n[2], g_n[4],m_n[2], m_n[4]);
        std::cout << "M = "<< std::endl << " "  << testPointt << std::endl << std::endl;
        cv::viz::WSphere point3DD(cv::Point3d(testPointt(0),testPointt(1), testPointt(2)), 2, 30, cv::viz::Color::yellow());
